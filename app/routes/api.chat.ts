...
async function chatAction({ context, request }: ActionFunctionArgs) {
  const { messages, files, promptId, contextOptimization, supabase, chatMode, designScheme, maxLLMSteps, selectedAgent } =
    await request.json<{
      messages: Messages;
      files: any;
      promptId?: string;
  ...
  try {
    ...
    const dataStream = createDataStream({
      async execute(dataStream) {
        ...
        let agentType: AgentType = selectedAgent || 'general';
        let agentSelection;
        ...
        if (!selectedAgent && files) {
          ...
          // Analyze project context and select appropriate agent
          const projectContext = agentRouter.analyzeProjectContext(files, userRequest);
          agentSelection = agentRouter.selectAgent(projectContext, userRequest);
          agentType = agentSelection.selectedAgent;
          
          // Send agent selection annotation to frontend
          dataStream.writeMessageAnnotation({
            type: 'agentSelection',
            selectedAgent: agentSelection.selectedAgent,
            confidence: agentSelection.confidence,
            reasoning: agentSelection.reasoning,
            suggestedAgents: agentSelection.suggestedAgents,
            projectContext: {
              ...
              fileCount: Object.keys(projectContext.files).length
            }
          });
        }
